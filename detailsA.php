...
<br>   
   ...
   <td align="center"><strong>Enter Owner's UserID:</strong></td>
...
<?php
global $location,$price,$total,$_SESSION;
...
<?php
		...
		/*$p= "SELECT name,parking_slot_name FROM userinfo WHERE location='$location' ";
		$run_emaila= mysqli_query($con,$p);
	    if ($run_emaila)
		{  
	      header('location: detailsB.php'); 
		}
			
	else {
		 */
		// $p=  "SELECT u.name,u.parkingSlotName,u.cntNO,p.slotSize,p.price,u.bkashNO,u.location FROM 'userinfo' AS u  INNER JOIN 'parkingslot' AS p   ON u. userID=p.userID WHERE location='$location' ";
          
          $p=  "SELECT parkingSlotName, userID ,mailID, name,location ,cntNO ,slotSize ,price ,bkashNO  FROM userprofile  WHERE  userID='$userID'  ";		  
		  
		  // $p=  "SELECT * FROM userinfo  ";
...
<td><input type="number" name="nos" placeholder="Enter positive number" required="required"/> </td>
</tr> 
...
<tr>
<td align="center"><strong>Enter Owners UserID:</strong></td>
<td><input type="varchar" name="userID" placeholder="enter owner's  userID" required="required"/> </td>
</tr>

<tr>
<td align="center"><strong>Enter number slot  of you want to book:</strong></td>
<td><input type="number" name="nos" placeholder="enter number of slot" required="required"/> </td>
</tr> 
...
if (!$con)
{
  ...
  echo " <br>";
   ...
   //$query="INSERT INTO parkingslot ( userID) value('$userID') WHERE name='$name' ";
